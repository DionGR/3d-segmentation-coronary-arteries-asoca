...
from config.imagecas.transforms import train_post_transforms, val_post_transforms
from config.imagecas.swin_unetr import loader_params, train_params, inferer, btcv_model_path, optimizer_params, loss_function, MAX_EPOCHS
from utils.data_loaders.imagecas_loader import ImageCASLoader
...
# Necessary for large number of files, do not remove
...
""" Data Loading """
loader = ImageCASLoader(**loader_params)
train_loader, val_loader = loader.get_train_val_dataloaders()
...
val_handlers = [
    ...
    TensorBoardStatsHandler(log_dir="data/tensorboard/imagecas/training/",
                            output_transform=lambda x: None,
                            global_epoch_transform=lambda x: trainer.state.epoch),
    
    TensorBoardImageHandler(
        log_dir="data/tensorboard/imagecas/training/",
        batch_transform=from_engine(["image", "label"]),
        output_transform=from_engine(["pred"])
        ),
    
    CheckpointSaver(save_dir="data/checkpoints/imagecas/",
